...
if st.button("Search") and user_query.strip():
    with st.spinner("Searching..."):
        try:
            ...
            semantic_results = solr.search(f"content_text:({user_query}) OR title:({user_query})", **{
                "defType": "edismax",
                "qf": "content_text^2 title",
                "knn": f"{{!knn f=content_embedding topK=10}}{embedding_str}",
                "fl": "*,score",
                "rows": 10,
                "mm": "1<50%"  # Match at least 50% of the terms
            })

            # If no semantic+keyword match, fallback to pure keyword
